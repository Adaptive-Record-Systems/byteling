...
Deno.serve(async (req) => {
  try {
    ...
    const existing = await svc.entities.ProviderKey.filter({ owner_email: user.email, provider });
    if (existing?.[0]) {
      await svc.entities.ProviderKey.update(existing[0].id, fields);
    } else {
      await svc.entities.ProviderKey.create(fields);
    }

    return Response.json({ ok: true, provider, key_hint: keyHint, status: 'active' });
  } catch (error) {
